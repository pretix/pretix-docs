...
By default, pretix will use your event's default tax rate for payment fees.
If you want to charge no tax on payment fees, navigate to :navpath:Your event → :fa3-wrench: Settings → Payment:, and open the :btn:Advanced: tab.
Under "Tax handling on payment fees", select `Charge no taxes`.
