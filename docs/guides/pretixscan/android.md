...
In order to set up pretixSCAN on your device, you need a QR code or access token generated by the pretix backend.
If you do not have access to the pretix backend yourself, then you need to ask someone who does have that access to send you the QR code or access token.
...
By default, the app is now in entry scanning mode.
