...
Most legislation require you to pay taxes on any sales you make using pretix. 
...
This page displays a list of all tax rules for the event. 
...
Switch to the :btn:Address form: tab and verify that the boxes next to "Ask for invoice address" and "Ask for VAT ID" are checked. 
...
However, since pretix always checks custom rules top to bottom and only applies the first matching rule, the software will only apply the second rule from our list (Austria → "Any customer" → "Charge VAT") to the transaction. 
...
"Reverse charge" is a rule in European Union VAT legislation that specifies how to pay taxes if the seller and buyer of a good reside in different EU countries. 
If the buyer is registered as a VAT-paying business in their country, you charge them only the net price without taxes and state that the buyer is responsible for paying taxes appropriately. 
...
If you enable the reverse charge feature and specify your merchant country, then pretix will perform the following process whenever a customer places an order:
 ...
 1. The shop presents the customer with net or gross prices as configured for the applicable tax rule. 
...
!!! Note 
    ...
    This is unavoidable due to the flexible nature of price calculation. 
