...
Der Unterschied zu anderen Zahlungsdienstleistern besteht lediglich darin, dass es für Wertgutscheine keine spezielle Erweiterung gibt, da sie eine Kernfunktion der pretix-Software darstellen.
...
Die Anzahl der von Ihnen verkauften Wertgutscheine hat nichts mit der Gesamtzahl der Eintrittskarten zu tun, die Sie für Ihre Veranstaltung verkaufen wollen.
...
### Wertgutscheine für manuelle Erstattungen verwenden

Mit pretix können Sie eine Bestellung erstatten, indem Sie manuell einen Wertgutschein erstellen.
...
Klicken Sie unter der Unterüberschrift "Zahlungen" auf den Button :btn:Betrag erstatten:.

Wählen Sie den zu erstattenden Betrag und die gewünschte Aktion für die Bestellung aus und klicken Sie anschließend den Button :btn:Weiter:.
Geben Sie im Feld "Erstattungsbetrag" neben "Neuen Wertgutschein erstellen" einen Betrag größer als 0,00 ein.
Optional können Sie außerdem ein "Ablaufdatum" auswählen.

Sobald Sie den Button :btn:Erstattung durchführen: klicken, wird ein Wertgutschein mit dem angegebenen Wert erstellt.
Anschließend gelangen Sie auf eine Seite mit dem Titel "E-Mail senden", wo Sie die Möglichkeit haben, dem\*der Kund\*in eine E-Mail mit dem Wertgutschein-Code zu senden.
Für Erstattungen erstellte Wertgutscheine werden außerdem unter :navpath:Ihr Veranstalter → :fa3-credit-card: Wertgutscheine: angezeigt.

### Wertgutscheine manuell ausstellen

![Seite mit dem Titel "Neuen Wertgutschein erstellen" mit Eingabefeldern für Code, Wert und Ablaufdatum sowie für besondere Gutscheinbedingungen. Außerdem Auswahlmöglichkeiten für die Währung und die Angabe, ob der Wertgutschein als Test erstellt wird.](../assets/screens/gift-cards/create.de.png)
...
Navigieren Sie dafür zu: :navpath:Ihr Veranstalter → :fa3-credit-card: Wertgutscheine:.
...
!!! Hinweis
    Sie können einen einmal erstellten Gutschein nicht löschen (nur entwerten – siehe hierzu [Das Guthaben eines Wertgutscheins manuell ändern](#das-guthaben-eines-wertgutscheins-manuell-ändern)
    Wenn Sie diese Funktion lediglich vorab testen möchten, aktivieren Sie das Kontrollkästchen neben "Testgutschein".
    Dies bedeutet, dass der Wertgutschein nur gültig ist, solange sich Ihr Shop im Testmodus befindet.

    Sie können einen im Testmodus erstellten Wertgutschein nicht in einen tatsächlich gültigen Wertgutschein umwandeln oder umgekehrt.
...
Geben Sie einen "Wert des Wertgutscheins" ein, der größer als null ist.
Wählen Sie eine Währung, die mit der Währung der Veranstaltungen übereinstimmt, für die Sie den Wertgutschein einsetzen möchten.

Wenn Sie einen Wertgutschein für den Testmodus erstellen möchten, aktivieren Sie das Kontrollkästchen neben "Testgutschein".
Wenn Sie einen Wertgutschein für den Live-Shop erstellen möchten, lassen Sie dieses Kontrollkästchen deaktiviert.
Ein für den Testmodus erstellter Wertgutschein funktioniert nur im Testmodus
Ein Wertgutschein für den Live-Shop funktioniert hingegen nur, wenn der Shop live ist.

Der Standardwert in den Feldern "Ablaufdatum" hängt ab von der Option "Gültigkeit von Wertgutscheinen in Jahren" in den Veranstalter-Einstellungen.
...
Außerdem können Sie mit pretix Wertgutscheine manuell entwerten.
...
Bei einem Einkauf im Wert von 12,50 geben Sie beispielsweise `-12,50` ein und klicken den Button :btn-icon:fa3-plus::.
Dies fügt einen neuen Eintrag zur Liste der Transaktionen hinzu und die Eigenschaft "Aktueller Wert" des Wertgutscheins wird entsprechend angepasst.
...
Wurde der Wertgutschein irrtümlich erstellt, ziehen Sie den gesamten Wert ab.
Wenn Sie das tun und jemand danach versucht, den Wertgutschein in Ihrem Shop einzulösen, dann erscheint die Nachricht, dass das gesamte Guthaben des Wergutscheins aufgebraucht ist.
