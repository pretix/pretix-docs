...
For instance, if you want to implement a use case like the one described under [General usage](#general-usage), check the boxes next to your basic access tickets and leave all other boxes unchecked.
...
A merchandise table may seem like a very different use case from a check-in, but a check-in list is still useful for keeping track of who has preordered which merch, and who has picked it up already.
