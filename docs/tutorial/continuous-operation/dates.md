...
!!! Note
    ...
    Once there is at least one date with a different name, the calendar view will display all date names again.
    Follow along with the [next section](#creating-dates-for-special-events) and the issue will resolve itself.
...
Under "Products", we will select the products `Guided tour` and `Guided tour (discount)
